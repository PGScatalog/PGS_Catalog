  ...
  if (gwas_id) {
    ...
    .done(function (data) {
      ...
      if (data.publicationInfo) {
        ...
        if (data_pub.author) {
          pub_author = data_pub.author.fullname;
        }
        if (data_pub.pubmedId) {
          pubmed_id = '<a '+ext_link+' href="https://www.ncbi.nlm.nih.gov/pubmed/'+data_pub.pubmedId+'">PMID:'+data_pub.pubmedId+'</a>';
        }
      }
      $('#gwas_pub_title').html(pub_title);
      $('#gwas_pub_info').html(pub_author+'<span class="pl-3 pr-3">|</span>'+pubmed_id);
